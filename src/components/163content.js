...
class RCsinger extends Component {
	...
	componentDidMount() {
		...
		fetchPicture('/artist/list?cat=5001&limit=5').then(res => {
			Promise.all(res.artists.map(data => { return fetchPicture(`/user/detail?uid=${data.accountId}`) }))
				.then(res => {
					for (let i = 0; i < res.length; i++) {
						let info = {
							nickname: res[i].profile.nickname,
							iconUrl: res[i].profile.avatarUrl,
							info: res[i].profile.description
						}
						singer.push(info)
					}
					this.setState({
	...
	<li>
		<a className="sg-item" href="#">
			<div className="sg-icon">
				<img className="sg-icon" src={item.iconUrl} width="62" height="62" alt="" />
			</div>
			<div className="sg-info">
				<h4>
					<span className="sg-id">{item.nickname}</span>
				</h4>
				<p className="sg-name">{item.info}</p>
			</div>
		</a>
	</li>
)
